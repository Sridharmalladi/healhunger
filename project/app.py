...
with tab2:
    ...
    # If no search address or search failed, use the first food post location
    if not map_center and food_posts:
        first_post = food_posts[0]
        if 'latitude' in first_post and 'longitude' in first_post:
            map_center = [first_post['latitude'], first_post['longitude']]
            print(f"Map centered on first food post location: {map_center}")
    
    # Create map only if we have a valid center
    if map_center:
        print("\nCreating map...")
        ...
        for i, post in enumerate(food_posts):
            ...
            # Skip posts without valid coordinates or required fields
            if not all([
                post.get('latitude'),
                post.get('longitude'),
                post.get('food_type'),
                ...
                print(f"Skipping post {i+1} due to missing required fields")
                continue
            ...
            try:
                location = [float(post.get('latitude')), float(post.get('longitude'))]
        ...
        # Update the food posts list to only show valid posts
        food_posts = valid_posts
    else:
        st.info("Please enter an address to search or wait for food posts to be available.")
